...
class ReviewController extends Controller
{
    public function add($product_slug)
    {
        ...
        if($product)
        {
            ...
            return view('frontend.reviews.index',compact('product','verified_purchase'));
             ...
             return redirect()->back()->with('status',"Sản phẩm không còn tồn tại");
        }
    }
    public function create(Request $request)
    {
        ...
        { $user_review = $request->input('user_review');
            ...
            if($new_review)
            {
                return redirect('category/'.$category_slug.'/'.$prod_slug)->with('status',"Cảm ơn vì đánh giá của bạn");
            }
        }
       else
       {
    ...
    public function update(Request $request)
    {
        ...
        if($user_review != '')
        {
            ...
            if($review)
            {
                ...
                $review->update();
                return redirect('category/'.$review->product->category->slug.'/'.$review->product->slug)->with('status',"Cảm ơn vì đánh giá của bạn");
            }
            else
            {
                return redirect()->back()->with('status',"Sản phẩm không còn tồn tại");
        ...
        else
        {
            ...
            return redirect()->back()->with('status',"Bình luận đang trống");
        }
